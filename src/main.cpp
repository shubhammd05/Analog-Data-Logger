...
void SDIndicatorTask(void *parameter){
  while(1){
  if(SD.usedBytes() <= (0.5 * SD.totalBytes())){
  digitalWrite(LEDR, LOW);
  digitalWrite(LEDG, HIGH);
}
else if(SD.usedBytes() > (0.5 * SD.totalBytes()) && SD.usedBytes() < (0.75 * SD.totalBytes())){
...
void Channel1Task(void *parameter) {
  while(!digitalRead(SWITCH) && sdCardIsReady){
    C1Data = C1.Read(); 
...
void setup() {
  ...
  delay(1000); // This is to ensure all components as started properly
...
void loop(){
  ...
  if (uploadButtonPressed) {
    ...
    if (!isApActive) {
      ...
      DEBUG_PRINTF("Max Alloc Block: %d bytes\n", ESP.getMaxAllocHeap());
      swapLoggingGroups();
      startAccessPoint();
    }
    // else {
